...
def encoder(model: str = "answerdotai/ModernBERT-large", epochs: float = 2.0, batch: int = 16, lr: float = 3e-5,
            limit: int = 0, test_limit: int = 0, brier_weight: float = 1.0, name: str = "encoder", head_lr: float = 1e-3, pool: str = "span", no_amp: bool = False, attn: str = "sdpa", reference_compile: str = "auto", max_len: int = 8192, bench_n: str = "1,10,100", max_state: int = 512, data_dir: str = DATA, augment: float = 0.0, consistency: float = 0.0):
    argv = ["--model", model, "--augment", str(augment), "--consistency", str(consistency), "--epochs", str(epochs), "--batch", str(batch), "--lr", str(lr), "--brier-weight", str(brier_weight), "--head-lr", str(head_lr), "--pool", pool, "--attn", attn, "--reference-compile", reference_compile, "--max-len", str(max_len), "--bench-n", bench_n, "--max-state", str(max_state)]
    if no_amp:
        argv += ["--no-amp"]
    if limit:
